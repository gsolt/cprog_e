...
* V2.1.4	2010.09.21	Gergely Zsolt		Bük, Nestlé: speciális SC parancs 
 v2.0     2014.04.16 Gergely Zsolt	Vételi számláló nullázása nap végén
*****************************************************************************/
      ...
      switch (type)
      {
        ...
        case CB_MSG_TYPE:
			...
			fnWriteSPStatus(sCP.sCPR[site_inx].nSPOffsetLek, TOPICAL);
 			/* MOSCAD_sprintf(message,"Lek. foly. nullazas,site index: %d, nSPOffsetLek: %d",site_inx,sCP.sCPR[site_inx].nSPOffsetLek);
   			MOSCAD_error(message ); 				*/
...
void tx_command(void)
{
   ...
   for (nI=0;nI<sCP.nRtuNum && nI<MAX_RTU ;nI++)
   {
   	...
   	{
   		fnSCTblIndx(nJ, &nSCTblIndx, &nOffset, &p_col_SCAct);
   		if (p_col_SCAct[nJ-nOffset]==0 && nI!=76)
   		{
   		   	/*tx_buffer[0] = 16;						
   		...
   		} /* end if   (p_col_SCAct[nJ-nOffset]==0) */
   		
   		else if ( nI==76 ) /* Buk, Nestle */
   		{
   		fnSCTblIndx(nJ, &nSCTblIndx, &nOffset, &p_col_SCAct);
   		nSCAct = p_col_SCAct[nJ-nOffset];
   		
   		fnPrSCTblIndx(nJ, &nSCTblIndx, &nOffset, &p_col_SCPr);
   		nSCPr = p_col_SCPr[nJ-nOffset];
   		...
   		if (nSCAct != nSCPr)
   		{
			...
			/* Tavirat elkuldese */
		   		...
		   		/* Mindenkeppen vissza frissit */
   				p_col_SCPr[nJ-nOffset] = p_col_SCAct[nJ-nOffset];
...
short				*p_col_DCAct; 
int     DP1,DP2;          /* Szõny, Dunalys állásjelzései */

/* Kezdõértékek megadása - minden frontendnél más!!! ******************************************************************/
/* Az index meghatározza a DP és SC IEC címeket!!! */
																													/**/
...
TMOK_ID[2][0] =1252;						/* TMOK azonosítója a táviratban = DP offset */								/**/
...
/* 3. TMOK:  45-12 -> Kõszegdoroszló PV erõmû -----------------------*/															/**/
TMOKAllasjelzesOffsetek[3] = 347; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
...
/* 4. TMOK:  64-26 -> Ostffyasszonyfa biogáz -----------------------*/															/**/
TMOKAllasjelzesOffsetek[4] = 1253; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[4][0] =1253;						/* TMOK azonosítója a táviratban = DP offset */								/**/
ReteszesRTUIndex[4][0] = 78;			/* Ostfyyasszonyfa biogáz */													/**/
...
/* 5. TMOK:  74-24 -> Ostfyyasszonyfa biogáz -----------------------*/															/**/
TMOKAllasjelzesOffsetek[5] = 1254; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[5][0] =1254;						/* TMOK azonosítója a táviratban = DP offset */								/**/
ReteszesRTUIndex[5][0] = 78;			/* Ostfyyasszonyfa biogáz */													/**/
...
/* 8. TMOK:  30-55 -> Front end G: Kincsesbánya 03-100 PV erõmû, A Redindancia, Csór 65/1, 65/2 -----------------------*/															/**/
...
TMOK_ID[8][1] =3;						  /* TMOK azonosítója a táviratban = DP offset */								/**/
ReteszesRTUIndex[8][1] = 224;			/* A redundancia */													/**/

ReteszesTMOK_RTUNum[8] = 2;				/* Az adott indexû TMOK ennyi kábelköri állomással kommunikál */			/**/
...
/* 9. TMOK:  31-48 -> Front end G, Kincsesbánya 03-100 PV erõmû ,A redundancia: Csór 65/1, 65/2-----------------------*/															/**/
...
TMOK_ID[9][1] =4;						  /* TMOK azonosítója a táviratban = DP offset */								/**/
ReteszesRTUIndex[9][1] = 224;			/* A redundancia */													/**/

ReteszesTMOK_RTUNum[9] = 2;				/* Az adott indexû TMOK ennyi kábelköri állomással kommunikál */			/**/
...
/* 10. TMOK:  41-98 -> G front end, RTU: Kenéz 062-9 PV erõmû - ez nincs -----------------------*/															/**/
TMOKAllasjelzesOffsetek[10] = 0; /* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[10][0] =0;						  /* TMOK azonosítója a táviratban = DP offset */								/**/
ReteszesRTUIndex[10][0] = 223;			/* G front end */													/**/
ReteszesTMOK_RTUNum[10] = 0;				/* Az adott indexû TMOK ennyi kábelköri állomással kommunikál */			/**/
nReteszPar[10] = 1;                /* 1: tartozik hozzá DC parancs, 0: nem tartozik hozzá DC parancs */
nReteszOffset[10] = 10;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/	/**/

/* 11. TMOK:  31-32 -> E front end, RTU: Székesfehérvár, Csala -----------------------*/															/**/
...
ReteszesRTUIndex[11][0] = 183;			/* Székesfehérvár, Csala */													/**/
ReteszesTMOK_RTUNum[11] = 1;				/* Az adott indexû TMOK ennyi kábelköri állomással kommunikál */			/**/
nReteszPar[11] = 0;                /* 1: tartozik hozzá DC parancs, 0: nem tartozik hozzá DC parancs */

/* 12. TMOK:  42-50 -> E front end, RTU: Szentgotthárd, vízierõmû -----------------------*/															/**/
TMOKAllasjelzesOffsetek[12] = 1257; /* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[12][0] =1257;						  /* TMOK azonosítója a táviratban = DP offset */								/**/
ReteszesRTUIndex[12][0] = 220;			/* Szentgotthárd, vízierõmû */													/**/
ReteszesTMOK_RTUNum[12] = 1;				/* Az adott indexû TMOK ennyi kábelköri állomással kommunikál */			/**/
nReteszPar[12] = 0;                /* 1: tartozik hozzá DC parancs, 0: nem tartozik hozzá DC parancs */

/* 13. TMOK:  30-88 -> G: TMOK 36601 -----------------------*/															/**/
TMOKAllasjelzesOffsetek[13] = 109; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[13][0] =1275;						/* TMOK azonosítója a táviratban = DP offset */								/**/
ReteszesRTUIndex[13][0] = 223;			/* G front end */													/**/
ReteszesTMOK_RTUNum[13] = 1;				/* Az adott indexû TMOK ennyi kábelköri állomással kommunikál */			/**/
nReteszPar[13] = 1;                /* 1: tartozik hozzá DC parancs, 0: nem tartozik hozzá DC parancs */
nReteszOffset[13] = 11;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/


/* 14. TMOK:  21-34 -> G: Annavölgy PV -----------------------*/															/**/
TMOKAllasjelzesOffsetek[14] = 478; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[14][0] =1278;						/* TMOK azonosítója a táviratban = DP offset */								/**/
ReteszesRTUIndex[14][0] = 223;			/* G front end */													/**/
ReteszesTMOK_RTUNum[14] = 1;				/* Az adott indexû TMOK ennyi kábelköri állomással kommunikál */			/**/
nReteszPar[14] = 1;                /* 1: tartozik hozzá DC parancs, 0: nem tartozik hozzá DC parancs */
nReteszOffset[14] = 12;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 15. TMOK:  85-83 -> G: Almásfüzítõ 012 PV -----------------------*/															/**/
TMOKAllasjelzesOffsetek[15] = 483; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[15][0] =1282;						/* TMOK azonosítója a táviratban = DP offset */								/**/
ReteszesRTUIndex[15][0] = 223;			/* G front end */													/**/
ReteszesTMOK_RTUNum[15] = 1;				/* Az adott indexû TMOK ennyi kábelköri állomással kommunikál */			/**/
nReteszPar[15] = 1;                /* 1: tartozik hozzá DC parancs, 0: nem tartozik hozzá DC parancs */
nReteszOffset[15] = 13;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 16. TMOK:  85-01 -> G: Almásfüzítõ 012 PV -----------------------*/															/**/
TMOKAllasjelzesOffsetek[16] = 480; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[16][0] =1283;						/* TMOK azonosítója a táviratban = DP offset */								/**/
ReteszesRTUIndex[16][0] = 223;			/* G front end */													/**/
ReteszesTMOK_RTUNum[16] = 1;				/* Az adott indexû TMOK ennyi kábelköri állomással kommunikál */			/**/
nReteszPar[16] = 1;                /* 1: tartozik hozzá DC parancs, 0: nem tartozik hozzá DC parancs */
nReteszOffset[16] = 14;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/
...
nReteszOffset[17] = 15;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 18. TMOK:  Szõny, Dunalys -> G: Almásfüzítõ 012 PV -----------------------*/															/**/
DP1 = fnReadDPData(185, 0, 0, 0, 0);
DP2 = fnReadDPData(186, 0, 0, 0, 0);

if (DP1==2 && DP2==2)
  {
  fnWriteDPData(1258,2, 0, 0, 0, 0);  
  }
else
  {
  fnWriteDPData(1258,1, 0, 0, 0, 0);  
  }
  

TMOKAllasjelzesOffsetek[18] = 1258; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[18][0] =1285;						/* TMOK azonosítója a táviratban = DP offset */								/**/
ReteszesRTUIndex[18][0] = 223;			/* G front end */													/**/
ReteszesTMOK_RTUNum[18] = 1;				/* Az adott indexû TMOK ennyi kábelköri állomással kommunikál */			/**/
nReteszPar[18] = 1;                /* 1: tartozik hozzá DC parancs, 0: nem tartozik hozzá DC parancs */
nReteszOffset[18] = 16;             /* DC parancs és DP állapot offsete, ha tartozik hozzá DC parancs*/

/* 19. TMOK:  30-36 -> G: Tác 012/1_1,2 PV -----------------------*/															/**/
TMOKAllasjelzesOffsetek[19] = 309; 		/* Az állásjelzés offsete a DP adatbázisban */								/**/
TMOK_ID[19][0] =1287;						/* TMOK azonosítója a táviratban = DP offset */								/**/
ReteszesRTUIndex[19][0] = 223;			/* G front end */													/**/
...
[0]: 100
[1]: DP offsete											
2019.05.22: figyelembe veszi a retesz funkció bénítva állapotot                                                    */
/****************************************************************************/
void fnDP_LEK( unsigned char *rx_buf, int nSite_ID)
{
